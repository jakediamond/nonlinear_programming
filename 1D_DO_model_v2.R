...
parms <- c(
  ...
  C_down = 8, # downstream boundary concentration (mg/L)
  ...
  # Reactive parameters
  GPP_max = 2, # maximum GPP rate (g O2/m2/h)
  ...
  # Storage parameters
  A_stor_frac = 1, # fraction of channel area that is storage area (-)
  alpha = 1, # exchange coefficient (m3/hr)
  k_er = 1, # storage area respiration coefficient (-)
...
model <- function(time, state, parms, gpp_choice, er_loc_choice, er_choice){
  with(as.list(parms), {
    ...
    storage = switch(er_loc_choice,
                     ...
                     both = alpha * (A / A_s) * (DO - DO_stor) + er * chan_frac
                     )
    
    # Change in stream DO concentration due to transfer to transient storage
    storage_str = alpha * (DO_stor - DO)
    
    # Rates of change
    dDO = switch(er_loc_choice,
                 storage = (adv_dis + (gpp + reaeration + 
                                         storage_str) / d) * del_t,
                 channel = (adv_dis + (gpp + reaeration + 
                                         storage_str + er) / d) * del_t,
                 both = (adv_dis + (gpp + reaeration + 
                                      storage_str + 
                                      er * (1 - chan_frac)) / d) * del_t,
                 )
