...
parms <- c(
  ...
  dx = 300, # segment length (m)
  L = 60000, # reach length (m)
  ...
  S = 0.01, # average channel slope (m/m)
  ...
  k_O2 = 8, # reaeration coefficient (1/d)
...
model <- function(time, state, parms, gpp_choice, er_loc_choice, er_choice){
  with(as.list(parms), {
    ...
    # Output of model
    return(list(diffs, fluxes, PAR = rep(par, N), ad = adv_dis))
  })
}  # end of model
...
# Plot the data
ggplot(data = filter(df, reach %in% c(95)),
       aes(x = time_hr,
           y = value,
           color = dist)) +
