...
model <- function(time, yini, parms){
  with(as.list(parms), {
    ...
    k = k_O2 * d # reaeration velocity (m/s)
    ...
    par = ifelse(540+800*sin(2*pi*time/24-1.4) < 0,
                 0,
                 540+800*sin(2*pi*time/24-1.4)) # (J/m2/s)
    reaeration = k * (do_sat - DO) # (mg O2/m2/s)
    gpp = (k_gpp * par) # (mg O2/m2/s)
    ...
    return(list(c(dDO = dDO, dDO_stor = dDO_stor)))  # results
  })
}  # end of model
...
# 2*N+2:3*N+1 = GPP; and 3*N+2:4*N+1 = ER
as_tibble(out) %>%
  # rename(2:51,) %>%
